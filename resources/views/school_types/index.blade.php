<x-app-layout>
    ...
    <div class="container">
        ...
        <div class="d-flex flex-row-reverse mb-4">
            <a href=" {{ route('school_types.create') }} " type="button" class="btn btn-primary" role="button">
        ...
        <div class="table-container table-responsive">
            ...
            <table class="table table-striped">
                ...
                @foreach($school_types as $school_type)
                    <tr>
                        <td>{{ $school_type->id }}</td>
                        <td>{{ $school_type->name }}</td>
                        <td>{{ $school_type->created_at }}</td>
                        <td>{{ $school_type->updated_at }}</td>
                        <td>{{ $school_type->deleted_at }}</td>
                        <td>
                            <div class="btn-group" role="group" aria-label="action buttons">

                                <x-datatables.action-link class="btn btn-primary"
                                                          url="{{ route('school_types.edit', $school_type) }}" {{-- atrybut url przekazywany do komponentu --}}
                                                          title="{{ __('translations.labels.edit') }}">
                                    <i class="bi-pencil"></i>
                                    </x-action-link>

                                    @if( $school_type->deleted_at == null )
                                        <x-datatables.confirm
                                            :action="route('school_types.destroy', $school_type)" method="DELETE"
                                            ...
                                            icon="question"
                                            :message="__('translations.categories.labels.destroy-question', ['name' => $school_type->name] )"
                                            button-class="btn btn-danger" :button-title="__('translations.labels.destroy')">
                                            <i class="bi bi-trash"></i>
                                            </x-confirm>
                                            @else
                                                <x-datatables.confirm
                                                    :action="route('school_types.restore', $school_type)" method="PUT"
                                                    :confirm-text="__('translations.buttons.yes')" confirm-class="btn btn-success me-2"
                                                    :cancel-text="__('translations.buttons.no')" cancel-class="btn btn-secondary ms-2"
                                                    icon="question"
                                                    :message="__('translations.categories.labels.restore-question', ['name' => $school_type->name] )"
